...
describe("Item Purchase", () => {
  ...
  it("user can travel to the needed section and add items to cart", () => {
    ...
    // hover over item 1
    cy.contains(/white wolf/i)
      .parent()
      .trigger("mouseover");

    // add item 1
    cy.contains(/white wolf/i)
      .siblings("button", { name: /add to cart/i })
      .click()
      .click();

    // hover over another item 2
    cy.contains(/comfy triangle/i)
      .parent()
      .trigger("mouseover");
    cy.contains(/comfy triangle/i)
      .parent()
    ...
    cy.get(".cart-item").each(($el) => {
      ...
      cy.wrap($el).within(() => {
        ...
        cy.get(".item-price")
          .then(($item) => (price = +$item.text().replace(/\$/g, "")))
          .then(() => (expectedTotal += quantity * price));
      });
    });
